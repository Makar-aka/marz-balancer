...
async def poll_loop():
    async with aiohttp.ClientSession() as session:
        while True:
            try:
                print("Обновляем данные о нодах...")
...
async def _fetch_nodes(session: aiohttp.ClientSession, token: Optional[str]) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            print("Полученные данные о нодах:", nodes)
            return nodes
    except Exception as e:
        print("Ошибка при получении данных о нодах:", e)
        return None
...
async def index(request: Request):
    ...
    print("Отладка: stats['nodes'] =", nodes)
